...
void SelectSPICEevents(){

  ReadCPTemp();
  Int_t stationId=2;
  Int_t runNum=12567;
  TString InputFileAddress="../../../ARA_Stuff/TestInterSPICEdata/run_012576/event012576.root";
  TString OutputFileAddress="Run12576_Stats.root";
  ...
  TFile *inFile=new TFile(InputFileAddress);

  ///Open the output root file to store the data stats
  TFile *outFile=new TFile(OutputFileAddress,"RECREATE");
   
  ///Open the Tree in the file that contains data from all of the events
  TTree *ceventTree = (TTree*)inFile->Get("eventTree");
  ...
  eventList+=runNum;
  eventList+=".txt";
  const char *filename=eventList.Data();    
  ...
  for(Int_t ievt=5; ievt<nument; ievt++){
    ...
    cout<<ievt<<" "<<rawAtriEvPtr->eventNumber<<endl;
    ...
    if(rawAtriEvPtr->isCalpulserEvent()==false && rawAtriEvPtr->timeStamp>1000 && rawAtriEvPtr->isSoftwareTrigger()==false){
      ///Initialise the Useful event pointer to load the ARA event waveforms witah the right calibration
  ...
  c1->BuildLegend();
  TString savePlot="AvgPower_AllCh_A";
  savePlot+=stationId;
  savePlot+="_SPICE_";
  savePlot+=runNum;

  TString PNG=savePlot;
  PNG+=".png";
  filename=PNG.Data();    
  c1->SaveAs(filename);

  TString PDF=savePlot;
  PDF+=".pdf";
  filename=PDF.Data();
  c1->SaveAs(filename);
  ...
  savePlot="AvgPowerNCorScore_AllCh_A";
  savePlot+=stationId;
  savePlot+="_SPICE_";
  savePlot+=runNum;
  ...
  savePlot="AvgPowerNCorScore_AllCh_colz_A";
  savePlot+=stationId;
  savePlot+="_SPICE_";
  savePlot+=runNum;
